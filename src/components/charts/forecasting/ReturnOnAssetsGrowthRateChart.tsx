...
const ReturnOnAssetsGrowthRateChart = ({data}: {data: Metric[]}) => {
  const isDarkMode = useAppSelector(selectDarkMode);
  const chartSeries = convertToChartSeries(data, "returnOnAssetsGrowthRate"); 
  ...
  const chartOptions = {
    ...
    series: chartSeries.map((series, index) => ({
        ...
        fillColor: {
          ...
          stops: [
            [0, series.color || 'rgba(255, 99, 71, 0.5)'],
            [1, series.color ? Highcharts.color(series.color).setOpacity(0.1).get() : 'rgba(255, 99, 71, 0.1)'],
          ],
        },
    })),
    plotOptions: {
...
export default ReturnOnAssetsGrowthRateChart;
